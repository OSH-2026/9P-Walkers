...
如果上层需要“同一条 UART 上双方都能随时主动发请求、并可靠区分响应和对端主动请求”，
不应继续直接使用这里的 `request()` / `serve_once()` 组合，而应把它放到共享 mesh 层之下：

- 主机侧由 `mesh_host_runtime` 在等待目标 `R*` 时继续处理途中帧；
- 节点侧由 `mesh_node_runtime + mesh_processer` 处理本地 `T*` 请求并回发响应；
- 本模块只保留 raw frame 收发职责，不再承担双向分发器角色。

也就是说，`uart_transport` 负责原始 frame I/O，`peer_link` 负责“同链路双向主动请求”的协议分发。
