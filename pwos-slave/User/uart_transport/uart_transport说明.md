...
不应继续直接使用这里的 `request()` / `serve_once()` 组合，而应在其上方接入共享层
`pwos-shared/mini9p/mini9p_peer_link.{c,h}`。

`peer_link` 会在等待本端响应时继续消费链路，并把：

- 匹配本端 pending tag 的 R* 返回给本端请求者
- 对端主动发来的 T* 交给本地 server 处理并回发响应
- 其他外来 R* 交给可选观察器或忽略
