 ...
 *
 * 本文件实现 3 条 FreeRTOS 静态队列，用于在 ISR、link_rx_task、
 * mesh_ctrl_task、link_tx_task 之间传递 pwos_frame_block_t 指针。
 *
 * 内存布局
 * --------
 * 每条队列需要两块静态内存：
 ...
 *   - mesh_rx / link_tx 队列：只应由任务调用 send/receive。
...
int pwos_queues_init(void)
{
    ...
    if (g_link_rx_queue == NULL || g_mesh_rx_queue == NULL || g_link_tx_queue == NULL) {
        return -1;
    }
    return 0;
}
