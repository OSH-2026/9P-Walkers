...
> **说明**：本节为**真实双机**分布式推理，需要两台同处一个局域网的机器（一台作主机 head，一台作从机 worker）。下文给出完整的配置步骤；标【待实测】处待第二台机器就绪后补充运行日志与数据。
...
1. 两台机器接入**同一局域网**（同一路由器 / 热点），互相能 `ping` 通：
   ...
   ```powershell
   ping 192.168.1.20   # 在主机上 ping 从机；反向也测一次
   ```
...
**运行结果（待实测）**：连接成功后，主机日志会出现 RPC 设备注册、且各层被分配到 `RPC0`（从机）计算，可证明从机确实参与推理；同时从机的 `rpc-server` 窗口会打印收到连接与计算请求。预期形如：

```
llama_prepare_model_devices: using device RPC0 (192.168.1.20:50052) (unknown id) - <从机可用内存> MiB free
load_tensors: layer  0 assigned to device RPC0, is_swa = 0
load_tensors: layer  1 assigned to device RPC0, is_swa = 0
... (其余层同样 assigned to device RPC0)
（llama-bench 结果中 backend 一列显示为 "RPC"，亦可佐证）
```

> 模型权重在加载阶段被下发到从机的 `rpc-server` 进程，因此**主机进程（llama-cli）的峰值内存会明显下降**（见 2.7 实测）。
> 【待补充：主机连接成功日志截图、从机 rpc-server 窗口截图】

### 2.7 单机 vs RPC 分布式推理性能对比

> 本节数据需在 2.6 的**真实双机**环境下采集。从机已按 2.6 启动 `rpc-server` 后，在**主机**执行下面一条命令即可同时测“仅本机”和“本机+从机(RPC)”两组，结果存于 `scripts/results/27_*`。
...
| 配置                         | 加载时间 (ms) | Prefill pp512 (t/s) | Decode tg128 (t/s) | TTFT (ms) | 主进程峰值内存 (MB) |
| ---------------------------- | ------------- | ------------------- | ------------------ | --------- | ------------------- |
| 单机（仅主机, CPU 后端）     | 【待填】      | 【待填】            | 【待填】           | 【待填】  | 【待填】            |
| RPC（主机 + 1 从机, 局域网） | 【待填】      | 【待填】            | 【待填】           | 【待填】  | 【待填】            |

**预期分析（实测后用真实数值复核）**：在双机 + 普通局域网（尤其 Wi-Fi）下，RPC 预计总体是**开销**而非提速，但能换来**内存横向扩展**。关注以下几点：

- **加载时间预计变长**：单机直接 mmap 本地权重即可；RPC 需把 4.36 GiB 权重**序列化后经网络下发**到从机，网络带宽越低、增幅越大。
- **Prefill 预计明显下降**：预填充计算密集、张量大，每层激活值都要**跨机传输 + 同步等待**从机算完，网络往返与序列化开销吃掉收益，是 RPC 路径最敏感的部分；网络越慢降得越多。
- **Decode 预计受影响较小**：decode 每步只传单 token 的小张量，传输量低；decode 本就内存带宽受限，RPC 额外开销相对不显著。
- **主进程峰值内存预计大幅下降**：这是 RPC 最有价值的现象——**模型权重驻留在从机的 rpc-server 进程**，主机只保留编排与 KV/计算缓冲。这正是 RPC 在真实多机中的意义：**把大模型拆到多台机器上，单机内存装不下的模型可借从机内存跑起来**。
- **结论方向**：RPC 的价值不在“提速”，而在**内存/显存的横向扩展**（用多机内存承载更大模型）。真实多机若走 Wi-Fi / 低带宽网络，prefill 会因逐层激活值传输进一步变慢（网络成为瓶颈）；同步等待与两机性能差异（木桶效应）也会放大开销。
- 【待补充：填入真实数值后，逐项给出实测倍率/百分比并与上述预期对照】
