...
因此，若希望同一条 UART 上双方都能随时主动发请求，raw transport 之上必须再加一层链路分发器。当前仓库中的对应实现是 `mini9p_peer_link`，它遵循以下规则：

- 收到匹配本端 pending tag 的 R*，交还给本端请求者。
- 收到对端主动发来的 T*，立刻交给本地 server 处理并回发响应。
- 收到其他不匹配当前 pending tag 的 R*，交给上层观察器或直接丢弃。

也就是说，`Tag` 仍只负责“请求与对应响应配对”，而“本帧是本端响应还是对端主动请求”则首先由 `Type` 区分。
