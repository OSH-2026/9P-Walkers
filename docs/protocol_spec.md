...
v1 的消息类型已经天然区分了 T* 请求和 R* 响应，因此“双方都能主动发请求”并不需要再改帧头字段。

真正的问题在于链路消费顺序：

- 本端在等待某个 tag 的 R* 响应时，下一帧未必是自己的响应。
- 它也可能是对端主动发来的 T* 请求。

因此，若希望同一条 UART 上双方都能随时主动发请求，raw transport 之上必须再加一层链路分发器。

当前仓库已经把这件事上移到 mesh 运行时完成，而不再保留独立的 `mini9p_peer_link` 模块：

- 主机侧由 `mesh_host_runtime_client_request()` 在等待目标 `R*` 响应时继续处理途中到来的 mesh 控制帧和其他数据面帧；
- 节点侧由 `mesh_node_runtime + mesh_processer` 把本地 `T*` 请求分发给 `m9p_server_handle_frame()`，再把响应重新封回 mesh MINI9P 帧；
- `Tag` 仍只负责“请求与对应响应配对”，而“本帧是本端响应还是对端主动请求”则先由 Mini9P `Type` 和外层 mesh 分流共同决定。
...
- 同一时刻，双方都可以各自使用自己的 tag 空间主动发起请求。
- 即使双方恰好使用了相同的 tag 值，也不会混淆，因为链路分发时先看 `Type`：
    - `T*` 一定是“对端主动请求”。
    - `R*` 一定是“某个已发出请求的响应”。
- 真正需要进一步匹配的只有 R*：即它是否对应本端当前等待的 tag。
