...
> - `pwos-master-esp32s3/inference/inference_runtime.cpp`
> - `pwos-master-esp32s3/inference/dist_inference_service.c`
...
## Part D: pwos_coordinator_runtime.c 安全加固（共 6 项）

### 修复 #1 [高] coordinator_task / probe_task 栈溢出风险
**问题**: `coordinator_task`（4096B）处理 9P/RPC/Job 协议栈 + ESP_LOG 调用链；`probe_task`（4096B）调用 `run_cluster_health_probe` → `pwos_cluster_vfs_read_path` → session 层。4KB 极易栈溢出。
**修复**: coordinator 栈增至 6144，probe 栈增至 5120。

### 修复 #2 [高] lock/unlock 回调未对 ctx/mutex 做空指针防御
**问题**: 所有 `*_lock`/`*_unlock` 回调直接将 `ctx` 强转为 `runtime*` 并访问成员，若配置错误传入 NULL 即 crash。`session_pending_wait` 也缺少 `runtime` 和 `pending_event[i]` 的空检查。
**修复**: 所有 12 个回调函数添加 `runtime != NULL` 和对应 mutex/semaphore 的 NULL 检查。

### 修复 #3 [中] `find_rpc_route` 遍历 cluster_vfs 无锁保护
**问题**: `coordinator_task` 中 `sync_cluster_vfs` 会并发修改路由表，而 `find_rpc_route`（被 RPC/Job 外部 API 调用）直接遍历无锁。
**修复**: 遍历前 `cluster_vfs_lock`，遍历后 `cluster_vfs_unlock`；同时增加 `route.target[0] == '\0'` 防御空字符串 `strcmp`。

### 修复 #4 [中] `run_cluster_health_probe` 未检查 target 空字符串
**问题**: 仅检查 `route == NULL`，若 `route->target[0] == '\0'`，`snprintf` 生成 `//sys/health`，虽不崩溃但语义错误。
**修复**: 增加 `route->target[0] == '\0'` 检查。

### 修复 #5 [低] RPC/JOB 响应帧传递路径不一致（已确认无 bug，添加文档）
**确认**: `PWOS_RPC_KIND_RESPONSE` 传递完整链路帧（`view->payload` 含 RPC 头），STREAM_CHUNK 仅传递解码后的 body（`rpc_view.payload`）。这是设计意图：session 层对一次性响应需要完整帧用于转发，对流式分块只需要 body 拼接。
**修复**: 添加注释文档说明此差异，防止误读为 bug。

### 修复 #6 [低] 错误路径不清理子系统资源（已知限制，未修改）
**现状**: `pwos_coordinator_runtime_start_default()` 失败时调用 `delete_runtime_sync`（清理 semaphore）后 `memset(&g_runtime, 0, ...)`，但 `parser`/`sessions`/`rpc_client`/`jobs`/`cluster_vfs` 等嵌入子模块的内部资源（若有 `malloc`）将泄漏。当前系统设计为一次性初始化，启动失败即重启，因此可接受。若有热重载需求需补充各子系统的 deinit 逻辑。
