...
| AI | 路由表/拓扑方向 | 9P 协议改造方向 | 命名/注册方向 | 总体倾向 |
|---|---|---|---|---|
| Kimi-2.6-pro | 主机维护全局邻接图并算路；从机只维护本地邻居表和下一跳缓存 | 复用 Mini9P 帧格式，新增 `TDISCOVER/RDISCOVER/TASSIGN/RASSIGN` 等控制类型；控制面独立处理，不进入文件操作回调 | 主机通过 `TASSIGN` 分配名称，从机持久化到 `/sys/node_name`；不复用 attach | 偏“扩展 Mini9P 类型 + 控制面分离”的保守方案 |
| GPT-5.5-Xhigh | 主机维护全局图；节点只维护本地邻居表和可选转发表，由主机下发路径/下一跳规则 | 在 UART transport 和 Mini9P 之间增加 mesh/network envelope；控制帧与 Mini9P payload 分离；明确反对复用 `Rattach.qid` | 设备先用稳定 `node_uid` 注册，主机返回 `NAME_ASSIGN`；后续 9P attach 只负责文件服务会话 | 偏“9P 外层网络 envelope”的传输/控制层分离方案 |
| Deepseek-v4-pro | 混合/SDN 风格：Master 维护完整链路状态图；Slave 只维护 `next_hop` 转发表，可考虑 Master 不可达时降级自治 | 推荐独立路由头 + 中间层分发；9P 完全不改，中间节点只看路由头转发；反对新增 9P 类型作为主方案，也反对改造 Rattach | 必须在 attach 前通过独立路由控制帧完成发现、硬件 ID 上报、`NAME_ASSIGN` 下发 | 偏“独立路由头 + 控制帧”的最强职责分离方案 |

| 问题 | 共识 | 主要分歧 |
|---|---|---|
| 是否所有节点维护全局拓扑 | 不建议。三者都认为主机维护全局拓扑/最短路，从机只保留局部邻居和下一跳表更合适 | Deepseek 额外提出 Master 不可达时可做距离矢量降级自治 |
| 路由计算放在哪里 | 主机侧集中计算，故障后重算并下发路由/下一跳 | Kimi/GPT 更强调简单集中式；Deepseek 更强调 SDN + 可降级 |
| 控制面是否进入 9P 文件语义 | 都不赞成把路由/命名塞进文件操作语义，尤其反对复用 `Rattach.qid` | Kimi 接受“新增 Mini9P 控制类型”；GPT/Deepseek 更倾向 9P 外层 envelope/独立路由头 |
| 从机如何获得名称 | 都赞成“硬件 ID/UID 注册，主机分配全局唯一名称，再下发给从机” | 下发通道不同：Kimi 用新增 Mini9P 控制帧；GPT/Deepseek 用外层控制协议/路由控制帧 |
| attach 是否承担命名 | 都不建议 | Deepseek 认为存在明确时序矛盾：命名应发生在 attach 之前 |

**一眼结论：** 路由和命名方向基本达成共识：采用“主机全局拓扑 + 从机 next_hop 转发 + 主机命名”的架构。真正需要团队拍板的是协议层：如果想少造一层协议，选 Kimi 的“Mini9P 新控制类型”；如果想让 mesh 转发和 9P 文件语义长期解耦，选 GPT/Deepseek 更支持的“外层 envelope/独立路由头”。
