...
/*
 * F429I-DISCO mesh serial port configuration:
 *   port 0 = UART5  (PC12 TX / PD2 RX, DMA1 Stream0/7)
 *   port 1 = USART6 (PG14 TX / PG9 RX, DMA2 Stream1/6)
 * USART1 (PA9/PA10) is reserved for ST-Link VCP debug console.
 */
#define PWOS_UART_DMA_PORT_COUNT 2u
...
static pwos_uart_dma_port_t g_ports[PWOS_UART_DMA_PORT_COUNT] = {
    { .desc = { .id = 0u, .name = "UART5", .huart = &huart5 }, .rx_buffer = g_uart_dma_rx_buffers[0] },
    { .desc = { .id = 1u, .name = "USART6", .huart = &huart6 }, .rx_buffer = g_uart_dma_rx_buffers[1] },
};
